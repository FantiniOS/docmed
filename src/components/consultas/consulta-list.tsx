...
import { Clock, User, Stethoscope, Trash2, CalendarClock, History } from "lucide-react";
...
export function ConsultaList({ initialConsultas }: { initialConsultas: ConsultaComRelacionamentos[] }) {
  ...
  const futuras = consultas
    .filter(c => new Date(c.data_consulta) >= hoje)
  ...
  const renderConsulta = (consulta: ConsultaComRelacionamentos) => {
      ...
      {futuras.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 mb-1">
            ...
            <Badge variant="default" className="bg-blue-500 hover:bg-blue-600 text-[10px] h-5 px-1.5">{futuras.length}</Badge>
          </div>
          <div className="grid gap-2">
            {futuras.map(renderConsulta)}
          </div>
        </div>
      )}
      ...
      {passadas.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-sm font-semibold text-foreground">Consultas Realizadas</h2>
            <Badge variant="secondary" className="text-[10px] h-5 px-1.5">{passadas.length}</Badge>
          </div>
          <div className="grid gap-2">
            {passadas.map(renderConsulta)}
          </div>
        </div>
      )}
